...
FROM utilizadores u
JOIN utilizador_cargos uc ON u.id = uc.utilizador_id
JOIN cargos c ON uc.cargo_id = c.id
JOIN permissoes p ON c.id = p.cargo_id
WHERE u.id = 111111111111111111;

SELECT 1
FROM utilizador_cargos uc
JOIN permissoes p ON uc.cargo_id = p.cargo_id
WHERE uc.utilizador_id = 111111111111111111
